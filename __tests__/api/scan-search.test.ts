 ...
 *
 * Unit tests for POST /api/scan/search — the manual name-search fallback for
 ...
 *
 * The scanner token and query travel in the POST body (not the query string)
 * so they never leak into access logs, proxy logs, history or Referer headers.
 */
...
import { POST } from '@/app/api/scan/search/route'
...
function makeRequest(q: string | null, token: string | null = SCANNER_TOKEN): NextRequest {
  ...
  if (q !== null) body.q = q
  return new NextRequest('http://localhost/api/scan/search', {
    method: 'POST',
...
describe('POST /api/scan/search', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns 400 when the scanner token is missing', async () => {
    const res = await POST(makeRequest('Alice', null))
  ...
  it.each(['%%', '__', '**', '%', '*', '%_*'])(
    ...
    async (probe) => {
      const { client } = mockAdminClient()
      const res = await POST(makeRequest(probe))
      expect(res.status).toBe(200)
      expect((await res.json()).results).toEqual([])
      expect(client.from).not.toHaveBeenCalled()
    }
  ...
  it('escapes LIKE metacharacters embedded in a real query', async () => {
    ...
    const res = await POST(makeRequest('Al%_e'))
    expect(res.status).toBe(200)
  ...
  it('strips asterisks (PostgREST % alias) from the pattern', async () => {
    const { ilikeSpy } = mockAdminClient()
    const res = await POST(makeRequest('Ali*ce'))
    expect(res.status).toBe(200)
