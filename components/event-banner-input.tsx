...
export function EventBannerInput({ defaultValue }: EventBannerInputProps) {
  ...
  const isExternalUrl = !!(defaultValue && !defaultValue.includes('/storage/v1/object/public/banners/'))
  const [activeTab, setActiveTab] = useState<'upload' | 'url'>(isExternalUrl ? 'url' : 'upload')
