...
export async function GET(request: NextRequest) {
  ...
  const rows = payments.map((p) => [
    ...
    escapeCSV(p.amount_kobo != null ? Math.ceil(p.amount_kobo / 100).toString() : null),
    escapeCSV(p.platform_fee_kobo != null ? Math.ceil(p.platform_fee_kobo / 100).toString() : null),
    escapeCSV(p.organiser_amount_kobo != null ? Math.ceil(p.organiser_amount_kobo / 100).toString() : null),
